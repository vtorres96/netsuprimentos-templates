 ...
 $(document).ready(function () {
     ...
     $(".bg_venda_com_a_net .round").click(function (c) {
         ...
         "vtex" == $('[name="x-input-radio"]:checked').val() ? ($('.seja_parceiro [name="plataforma"]').hide(), $('.seja_parceiro [name="plataforma"]').val("VTEX")) : "dmCommerce" == $('[name="x-input-radio"]:checked').val() ? ($('.seja_parceiro [name="plataforma"]').hide(), $('.seja_parceiro [name="plataforma"]').val("DMCommerce")) : ($('.seja_parceiro [name="plataforma"]').show(), $('.seja_parceiro [name="plataforma"]').val(""))
     });
     ...
     $(".seja_parceiro form").submit(function (a) {
         ...
         errorPlacement: function (a, b) {
             "x-input-radio" == b.attr("name") ? a.appendTo(b.closest(".plataforma > label")) : a.insertAfter(b)
         },
         ...
         submitHandler: function (a) {
             if (!$(".inputButtons button").hasClass("finish")) {
                 ...
                 var b = {
                     ...
                     "fields[2][NomeFantasia]": $('.seja_parceiro [name="nFantasia"]').val(),
                     "fields[3][RazaoSocial]": $('.seja_parceiro [name="rSocial"]').val(),
                     "fields[4][CNPJ]": $('.seja_parceiro [name="cnpj"]').val(),
                     "fields[5][Site]": $('.seja_parceiro [name="site"]').val(),
                     "fields[6][Plataforma]": $('.seja_parceiro [name="plataforma"]').val(),
                 ...
                 $.ajax({
                     ...
                     type: "POST",
                     success: function (a) {
                         console.log(a), $(".inputButtons button").removeClass("active"), $(".inputButtons button").addClass("finish"), swal("Pronto!", "As informaÃ§Ãµes foram enviadas!", "success").then(function () {})
                     },
                     error: function (a) {
                         $(".inputButtons button").removeClass("active"), $(".inputButtons button").removeClass("finish"), swal("Erro!", "VocÃª jÃ¡ nos enviou as informaÃ§Ãµes favor aguardar!", "error").then(function () {})
                     }
                 })
             }
         }
