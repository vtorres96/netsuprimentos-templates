sliderBrands = function() {
    $("section#topMarcas").length > -1 &&
        ($("section#topMarcas .slider-wrapper").slick({
                slidesToShow: 7,
                slidesToScroll: 2,
                dots: !1,
                arrows: !0,
...
form.addEventListener("submit", function(ev) {
        ...
        var name = document.querySelector('.x-form-bf-news input[name="nome"]').value,
            ...
            segmento = document.querySelector('.x-form-bf-news select[name="segmento"]')
            .value;
        if (null != name && "" != name) {
            var formData = {
                Unique: "Email",
            ...
            $.ajax({
                ...
                success: function(res) {
                    1 == res.response ?
                        ...
                        (document.querySelector('.x-form-bf-news input[name="nome"]').value =
                            ""),
                        (document.querySelector('.x-form-bf-news input[name="email"]').value =
                            ""),
                        console.log(res);
                }
            });
